...
async function handleSubmit() {
  ...
  try {
    ...
    // ✅ 成功 toast（不使用 timeout，避免类型红线；如果你的 @nuxt/ui 支持可换成 duration）
    toast.add({
      title: '预约提交成功',
      description: '我们已收到您的请求，并向您的邮箱发送了确认邮件。',
      color: 'success'
      // durtion: 4000   // ← 如果你版本类型支持，再开启
    })
  } catch (e: any) {
    errorMsg.value = e?.data?.detail || e?.message || '提交失败'
    // ✅ 失败 toast（不改变原有 errorMsg 顶部提示）
    toast.add({
      title: '提交失败',
      description: errorMsg.value || '请稍后再试',
      color: 'error'
    })
  } finally {
    loading.value = false
  ...
  <UCard class="max-w-3xl mx-auto">
    ...
    <div class="space-y-6">
      ...
      <UFormField label="联系类型" class="min-w-0">
        <USelect v-model="form.contact_type" :items="CONTACT_TYPES" value-key="value" class="w-full"
      ...
      <div v-if="isOnline" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <UFormField label="首选联络方式 *" class="min-w-0">
          <USelect v-model="form.preferred_contact" :items="PREFERRED_CONTACT" value-key="value" class="w-full"
            :popper="{ strategy: 'fixed' }" />
        </UFormField>

        <UFormField label="方便联系时间 *" class="min-w-0">
          <USelect v-model="form.preferred_time" :items="PREFERRED_TIME" value-key="value" class="w-full"
            :popper="{ strategy: 'fixed' }" />
        </UFormField>
      </div>
      ...
      <div v-if="isInPerson" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <UFormField label="到店时段 *" class="min-w-0">
          <USelect v-model="form.visit_time" :items="VISIT_TIME" value-key="value" class="w-full"
            :popper="{ strategy: 'fixed' }" />
        </UFormField>

        <UFormField label="到店目的 *" class="min-w-0">
          <USelect v-model="form.visit_purpose" :items="VISIT_PURPOSE" value-key="value" class="w-full"
            :popper="{ strategy: 'fixed' }" />
        </UFormField>
      </div>
      ...
      <UAlert v-if="errorMsg" color="error" variant="soft" icon="i-lucide-alert-triangle" :description="errorMsg" />
      <UAlert v-if="successData" color="success" variant="soft" icon="i-lucide-badge-check" title="提交成功" />
    </div>
